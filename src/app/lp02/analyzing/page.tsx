...
export default function AnalyzingPage() {
  ...
  useEffect(() => {
    ...
    // LP02 では analyzing 後にいきなり result ではなく、
    // 「結果プレビュー + リードフォーム」ページ (/lp02/preview/[id]) に遷移する。
    // ここで簡易フォーム (氏名・電話・メール・生年月日任意) を取ってから
    // 詳細結果 (/lp02/result/[id]) を解放する 2 段階リードフロー。
    const doneId = window.setTimeout(() => {
      router.replace(`/lp02/preview/${resultId}`);
