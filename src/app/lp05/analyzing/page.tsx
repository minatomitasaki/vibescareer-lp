...
// =====================================================
// LP05 分析中ページ (/lp05/analyzing)
//   - 中央: SVG ニューラルネット図 (全アニメは SMIL <animate>)
//   - 3 秒カウントダウン後に /lp05/result/[id] へ自動遷移
//     (LP04 と違い preview 鍵ページを挟まず、フル詳細結果へ直接遷移する)
// =====================================================
...
export default function AnalyzingPage() {
  ...
  useEffect(() => {
    ...
    // LP05 は analyzing 後にそのまま詳細結果 (/lp05/result/[id]) へ。
    const doneId = window.setTimeout(() => {
      router.replace(`/lp05/result/${resultId}`);
  ...
  return (
    <main className="lp-container min-h-screen bg-white flex flex-col items-center justify-center px-6 relative overflow-hidden">
      ...
      <div className="relative w-[260px] h-[260px] mb-10">
        ...
        >
          {/* 走査リング 2 枚 */}
          <circle cx="100" cy="100" r="35" fill="none" stroke="#FF6B00" strokeWidth="1" opacity="0.5">
            <animate attributeName="r" values="35;95;35" dur="2.4s" repeatCount="indefinite" />
            <animate attributeName="opacity" values="0.55;0;0.55" dur="2.4s" repeatCount="indefinite" />
          </circle>
          <circle cx="100" cy="100" r="35" fill="none" stroke="#FF6B00" strokeWidth="1" opacity="0.5">
            <animate attributeName="r" values="35;95;35" dur="2.4s" begin="1.2s" repeatCount="indefinite" />
            <animate attributeName="opacity" values="0.55;0;0.55" dur="2.4s" begin="1.2s" repeatCount="indefinite" />
          </circle>

          {/* 静的ベースエッジ */}
          {EDGES.map((e, i) => (
            <line key={`base-${i}`} x1={e.x1} y1={e.y1} x2={e.x2} y2={e.y2} stroke="#FF6B00" strokeOpacity="0.15" strokeWidth="1" />
          ))}
          ...
          {EDGES.map((e, i) => (
            ...
            >
              <animate attributeName="stroke-dashoffset" from="108" to="-90" dur="1.6s" begin={e.begin} repeatCount="indefinite" />
            </line>
          ))}

          {/* 周囲ノード */}
          {NODES.map((n, i) => (
            <circle key={`node-${i}`} cx={n.cx} cy={n.cy} r="7" fill="#FFE5D1" stroke="#FF6B00" strokeWidth="1.5">
              <animate attributeName="r" values="7;9.5;7" dur="2.4s" begin={n.delay} repeatCount="indefinite" />
              <animate attributeName="fill" values="#FFE5D1;#FF6B00;#FFE5D1" dur="2.4s" begin={n.delay} repeatCount="indefinite" />
              <animate attributeName="stroke-width" values="1.5;2.5;1.5" dur="2.4s" begin={n.delay} repeatCount="indefinite" />
            </circle>
          ))}

          {/* 中央ハブ */}
          <circle cx="100" cy="100" r="11" fill="#FF6B00">
            <animate attributeName="r" values="11;13.5;11" dur="1.6s" repeatCount="indefinite" />
            <animate attributeName="fill-opacity" values="1;0.85;1" dur="1.6s" repeatCount="indefinite" />
          </circle>
