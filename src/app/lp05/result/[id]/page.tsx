// LP05 詳細結果ページ (/lp05/result/[id])
//
// 導線: 入口LP → 診断 → 診断中 → ここ(フル詳細表示) → LINE 登録CTA
//
// 構成は careerpad.jp/shindan/short/ を参考に以下の順に再編:
//   1. 診断結果 (ヒーロー + 持ち味/アドバイス/その他の適職)
//   2. CONCERNS (悩み訴求)
//   3. SUCCESS STORIES (転職成功事例)
//   4. CAREER ADVISORS (アドバイザー紹介)
//   5. FAQ
//   6. LAST MESSAGE (代表メッセージ)
//   7. LINE 登録CTA (末尾) + 下部固定 LINE CTA
//
// 診断結果はモザイク(鍵)にせず全文表示する。
// 結果分類・データ・LINE URL 解決は LP04 と共通のものを再利用。
...
import { Lp05StickyCta, Lp05GuideCta } from "@/components/lp05/Lp05LineCta";
import { RESULT_DATA, type ResultData, type ResultId } from "@/data/results";

// LP05 は 12 パターン全てを生成する (LINE URL が 1 本のため集約不要)。
const RESULT_IDS = Object.keys(RESULT_DATA) as ResultId[];
const isResultId = (id: string): id is ResultId => id in RESULT_DATA;
import {
  ADVISORS,
  SUCCESS_CASES,
  type Advisor,
...
// -----------------------------------------------------------------------------
// 静的生成: LP04/LP05 共通の 3 パターン (sales-stable / marketing-stable / planning-stable)
// -----------------------------------------------------------------------------
export function generateStaticParams() {
  return RESULT_IDS.map((id) => ({ id }));
}
...
export default async function ResultPage({
  ...
  return (
    ...
    <main className="lp-container bg-white pb-40">
      ...
      <GuideBonusSection resultId={data.id} />
      ...
      <GuideShortCta resultId={data.id} bg="bg-bg-subtle" />

      {/* 4. CAREER ADVISORS */}
      <AdvisorsSection />

      {/* 短縮CTA (アドバイザー紹介の下・背景は白で密着) */}
      <GuideShortCta resultId={data.id} bg="bg-white" />
      ...
      {/* 7. 結果LP末尾: 特典CTAセクション (旧フォーム位置・「そんなあなたへ！」は非表示) */}
      <GuideBonusSection resultId={data.id} showHeadline={false} />

      {/* 下部固定 CTA: LINE 登録 (常時表示) */}
      <Lp05StickyCta resultId={data.id} />
    </main>
  );
}
  ...
  return (
    <section className="bg-gradient-to-b from-[#F0FDF4] to-[#DCFCE7] px-4 pt-8 pb-10">
      ...
      <div className="cta-radar-stack max-w-[330px] mx-auto mt-4">
        ...
        <Lp05GuideCta resultId={resultId}>
          「20代転職ガイド」を
          <br />
          いますぐ受け取る！
        </Lp05GuideCta>
